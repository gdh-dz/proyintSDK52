...
import { getProductById } from '@/services/products'; // Asegúrate de tener este servicio que obtiene el producto

const EditProductScreen: React.FC = () => {
    const { id } = useSearchParams<{ id: string }>(); // Obtener el id del producto desde los parámetros del enrutamiento
    const [product, setProduct] = useState<any>(null); // Estado para guardar el producto
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            if (id) {
                try {
                    const fetchedProduct = await getProductById(id); // Obtener el producto
                    if (fetchedProduct) {
